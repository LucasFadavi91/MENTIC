...
# Función : comprobarEmailRepetido()
# Parametros: $email
#  
# 
# Funcionalidad: 
#  Comprueba si existe un email de usuario
#    
# retorna: boolean
...
function comprobarEmailRepetido($email) {
	...
	global $conexion;
		...
		$select = $conexion->prepare("SELECT id_user FROM users WHERE email= :email");
		$select->bindParam(":email", $email);
		$select->execute();

				
		if($select->rowCount() != 0){ //si encuentra un email
